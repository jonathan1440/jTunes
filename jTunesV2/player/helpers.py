...
def new_genre(name: str, verbose: bool = False):
    """
...
def new_artist(name: str, genres: [Genre.name] = None):
    """
    name is required

    >>> new_artist('t Rameses B', ['t Trance']).name
    't Rameses B'
    """
...
def new_song(name: str, path: str = None, artists: [Artist.name] = None, remix_of: Song = None, composers: [Artist.name] = None,
    ...
    >>> new_song(name='t Lost', artists=['t Rameses B'], arousal=0.7, valence=0.5).name
    Traceback (most recent call last):
        ...
    TypeError: new_song() missing 1 required positional argument: 'path'
    >>> new_song(name='t Lost', path='H:/Music', artists=['t Rameses B'], arousal=0.7, valence=0.5).name
    't Lost'
    """
    ...
    if song_genres:
        for genre in song_genres:
            s.song_genres.add(new_genre(name=genre))
...
def new_album(name: str, album_artists: [Artist] = None, songs_and_track_nums: [[Song.name, int]] = None):
    """
    name is required
    songs_and_track_nums is an array of [Song, track_num] pairs. track_num can be None.

    >>> new_album(name='t Drift Away', album_artists=['t Rameses B']).name
    't Drift Away'
    """
...
def get_playlist_paths(playlist):
    paths = []
    [paths.append(song.path) for song in playlist.songs.all()]
    [[paths.append(song.path) for song in album.songs.all()] for album in playlist.albums.all()]
    [paths.extend(get_playlist_paths(p)) for p in playlist.playlists.all()]
    return paths
